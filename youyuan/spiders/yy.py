...
class YySpider(RedisCrawlSpider):
    name = "yy"
    #allowed_domains = ['youyuan.com'] #可选
    # start_urls = ['http://www.youyuan.com/find/chengdu/mm18-25/advance-0-0-0-0-0-0-0/p1/']
    ...
    # 第一级匹配规则：成都市18-25岁女性的每一页链接匹配规则
    page_links = LinkExtractor(allow=(r"youyuan.com/find/chengdu/mm18-25/advance-0-0-0-0-0-0-0/p\d+/"))
    # 第二级匹配规则 ：每个女性主页的匹配规则
    ...
    def get_education(self, response):
        education = response.xpath("//div[@class='pre_data']/ul/li[2]//ol[1]/li[3]/span/text()").extract()
        if len(education):
